...
//    function validateTransaction(bytes32 transactionId , bytes[] memory signatures ,bool mintable) external  {
   function validateTransaction(bytes32 transactionId ,bool mintable) external  {
    ...
    //       address signer = getSigner(getChainId() ,transaction.chainId , transaction.assetAddress , transaction.id , transaction.receiver , transaction.nounce , signatures[i]);
    ...
    //    require(validSignatures >= Isettings(Ibridge(owner()).settings()).minValidations() ,"insuficient_signers");
       transactionValidations[transactionId].validationCount = 1; 
   ...
   function getEthSignedMessageHash(
        ...
        returns (address) 
    {
        bytes32 ethSignedMessageHash =  getEthSignedMessageHash(
            chainID,
            interfacingChainId,
            assetAddress, id, receiver, nounce);
